...
export function WalletConnect() {
  ...
  const setShowSavedUsers = useGlobalStore(
    (globals) => globals.setShowSavedUsers,
  );
  ...
  async function sign() {
    ...
    await xmtp.initClient(signer, address.toLowerCase());
    await xmtp.initStream();
    ...
    // Below: save multiple accounts in localstorage
    ...
    // use rust and keyring to store pk
    // const result = await tryCatch(invoke("login", {user: pushApi.account, pk: pushApi.decryptedPgpPvtKey!}));
    // if(!result.error){
    //   console.log("Successfully stored user pk in keyring");
    // }else{
  ...
  return (
    <>
      <div
        className={`absolute top-0 w-full h-full duration-300 ${showSavedUsers ? "translate-x-full" : ""}`}
      >
        <button onClick={() => setShowSavedUsers(true)}>
          <img className="absolute left-5 top-1/2 z-10 w-14 h-14" src={arrow} />
        </button>
        ...
        <div className="flex flex-col w-full h-full justify-center place-items-center p-10 gap-2">
          <div className="text-deep-purple-100 text-2xl">
            Connect New Wallet
          </div>
          {/* <ConnectButton/> */}
          ...
          {connected ? (
            <div className="flex flex-col gap-1 justify-center place-items-center">
              ...
              <button
                className="flex h-16 w-40 bg-deep-purple-100 text-deep-purple-400 rounded-xl font-medium text-xl justify-center place-items-center"
                onClick={sign}
              >
                {loading ? (
                  <img
            ...
            <div className="pt-8">
              <button
                className="flex h-16 w-40 bg-deep-purple-100 text-deep-purple-300 hover:text-deep-purple-500 rounded-xl font-semibold text-xl hover:shadow-none duration-100 justify-center place-items-center"
                onClick={connect}
              >
