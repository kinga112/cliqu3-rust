...
type Action = {
  ...
  setTextChannels: (textChannels: TextChannelMetaData[]) => void;
  // appendTextChannel: (textChannel: TextChannel) => void;
  appendTextChannel: (textChannelMetaData: TextChannelMetaData) => void;
  ...
  appendMessage: (message: XMTPMessage) => void;
...
export const useServerStore = create<ServerType & Action>((set) => ({
  ...
  // setUserProfiles: (userProfiles: { [address: string]: UserProfile }) =>
  //   set(() => ({ userProfiles: userProfiles })),
  // setUsers: (users: {
  //   admins: Array<ChatMemberProfile>;
  //   members: Array<ChatMemberProfile>;
  // }) => set(() => ({ users: users })),
  setCurrentVoiceChannel: (chatId: string) =>
    set(() => ({ currentVoiceChannel: chatId })),
  // setReply: (reply: Reply | null) => set(() => ({ reply: reply })),
  setCurrentTextChannel: (textChannel: TextChannel) =>
    set(() => ({ currentTextChannel: textChannel })),
  setTextChannels: (textChannels: TextChannelMetaData[]) =>
    set(() => ({ textChannels: textChannels })),
  // appendTextChannel: (textChannel: TextChannel) =>
  //   set((server: ServerType) => ({
  //     textChannels: [...server.textChannels, textChannel],
  //   })),
  appendTextChannel: (textChannelMetaData: TextChannelMetaData) =>
    set((server: ServerType) => ({
      textChannels: [...server.textChannels, textChannelMetaData],
    })),
  setVoiceChannels: (voiceChannels: { [chatId: string]: VoiceChannel }) =>
    set(() => ({ voiceChannels: voiceChannels })),
  appendVoiceChannel: (voiceChannel: VoiceChannel) =>
    set((server: ServerType) => {
  ...
  setAddChannelModalVisibility: (visibility: boolean) =>
    set(() => ({ addChannelModalVisibility: visibility })),
  // clearMessages: () =>
  //   set((server) => ({ currentTextChannel: { id: currentTextChannel.id,  } })),
  setFiles: (files: Array<any>) => set(() => ({ files: files })),
  appendFile: (file: any) =>
    set((server: ServerType) => ({ files: [...server.files, file] })),
  // setMessages: (messages: Message[]) => set(() => ({ messages: messages })),
  // appendMessage: (message: Message) =>
  //   set((server: ServerType) => ({ messages: [...server.messages, message] })),
  appendMessage: (message: XMTPMessage) =>
    set((server: ServerType) => ({
      currentTextChannel: {
        id: server.currentTextChannel.id,
        name: server.currentTextChannel.name,
        description: server.currentTextChannel.description,
        messages: [...server.currentTextChannel.messages, message],
        members: server.currentTextChannel.members,
      },
    })),
  // appendNewMessages: (messages: Message[]) =>
  //   set((server: ServerType) => ({
  //     messages: [...server.messages, ...messages],
  //   })),
  // appendOldMessages: (messages: Message[]) =>
  //   set((server: ServerType) => ({
  //     messages: [...messages, ...server.messages],
  //   })),
  // addReferenceId: (id: string, cid: string) =>
  //   set((server: ServerType) => ({
  //     messages: server.messages.map((msg: Message) =>
  //       msg.id === id ? { ...msg, cid: cid } : msg,
  //     ),
  //   })),
  // addOrRemoveReaction: (emoji: string, user: string, cid: string) =>
  //   set((server: ServerType) => {
