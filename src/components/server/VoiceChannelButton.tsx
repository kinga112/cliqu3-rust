...
export function VoiceChannelButton(props: {
  ticket: string;
  id: string;
  name: string;
  active_users: string[];
  ...
  const setCurrentVoiceChannel = useGlobalStore(
    (globals) => globals.setCurrentVoiceChannel,
  );
  ...
  async function startCall() {
    if (props.active_users.length > 0) {
      ...
      const result = await tryCatch(
        invoke("start_call", {
          serverId: currentServer!.metadata.id,
          voiceChannelId: props.id,
          user,
        }),
      );
      if (!result.error) {
  ...
  async function endCall(user: string) {
    ...
    const result = await tryCatch(
      invoke("end_call", { serverId, voiceChannelId, user }),
    );
    if (!result.error) {
      console.log("RESULT DATA: ", result.data);
    }
  ...
  const users = props.active_users.map((user: string) => {
    return (
      <div className="flex justify-between hover:bg-off-black-400 rounded-sm p-1 select-none">
        <div>{user.slice(0, 8)}</div>
        <button
          onClick={() => endCall(user)}
          className="w-20 rounded bg-deep-purple-300"
        >
          End Call
        </button>
      </div>
  ...
  return (
    <>
      <div className="flex flex-col w-full border-deep-purple-300 overflow-y-auto px-2">
        <button
          className="flex w-full h-8 place-items-center p-1 hover:bg-off-black-400 rounded-lg"
          onClick={startCall}
        >
