...
import { EMOJIs, XMTPMessage } from "../../../types/messageTypes";
...
export function BottomBar() {
  ...
  // const inboxId = useUserStore(user => user.inboxId);
  ...
  const setKlipy = useGlobalStore((globals) => globals.setKlipy);
  ...
  function outsideGiphyAlerter(ref: React.RefObject<any>) {
    useEffect(() => {
      /**
  ...
  function outsideStickersAlerter(ref: React.RefObject<any>) {
    useEffect(() => {
      function handleClickOutside(event: MouseEvent) {
        if (ref.current) {
          if (!ref.current.contains(event.target)) {
  ...
  async function send() {
    ...
    // Need to fill this info in after success, well from should be inbox id, and id can be filled later!
    let newMessage: XMTPMessage = {
      id: "",
      content: input,
      content_type: "text",
      from: address,
    ...
    const result = await tryCatch(
      invoke("send_message", { id: currentTextChannel.id, text: input }),
    );

    if (result.error) {
      console.log("message failed to send: ", result.error);
    } else {
  ...
  //           const recipientProfile = await push.api!.profile.info({
  //             overrideAccount: chatId,
  //           });
  ...
  const imageList = images.map((src: string) => {
    return <ImageItem key={src} src={src} />;
  });
    ...
    return (
      <>
        <div className="inline-block relative group">
          ...
          <button
            className="absolute -top-2 -right-3 invisible group-hover:visible z-50"
            onClick={() => removeFile(props.file)}
          >
            <img
              className="hover:bg-red-700 rounded-full"
              src={close}
  ...
  const filesList = files.map((file, index) => {
    return <FileItem key={file.file} file={file} index={index} />;
  });
  // const emojiList = Object.values(CHAT.REACTION)
  const emojiList = Object.values(EMOJIs);
  let emojiElements = emojiList.map((emoji: string) => (
    <button
      key={emoji}
      className="hover:bg-deep-purple-400 rounded-md"
      ...
      <div className="flex w-full justify-center shrink-0">
        ...
        <div className="relative h-[calc(58px)] w-full mx-3">
          ...
          <div className="absolute bottom-2 w-full">
            <Reply />
            {filesList.length != 0 ? (
              <div className="absolute z-50 -top-1 -translate-y-full p-2 bg-deep-purple-300 rounded-md max-h-[500px]">
                {filesList}
              </div>
            ) : (
              <p />
            ...
            {images.length != 0 ? (
              <div className="absolute z-50 -top-1 -translate-y-full bg-deep-purple-300 rounded-md p-2 max-h-[500px]">
                {imageList}
              </div>
            ) : (
              <p />
            ...
            <textarea
              ...
              className="z-0 w-full min-h-14 bg-off-black-600 rounded-lg px-2 py-4 focus:outline-none pr-56 resize-none"
              placeholder={`Send message to #${currentTextChannel.name}`}
              value={input}
            ...
            <div
              ref={giphyRef}
              className={
                "absolute z-100 w-[500px] h-[500px] right-18 bottom-16 bg-deep-purple-100 rounded-xl " +
                showGiphy
              }
            >
              <Klipy />
            ...
            <div
              ref={stickerRef}
              className={
                "absolute z-100 w-[500px] h-[500px] right-18 bottom-16 bg-deep-purple-300 rounded-xl " +
                showStickers
              }
            >
              <Klipy />
            ...
            <div
              ref={emojiRef}
              className={
                "z-50 absolute right-5 bottom-14 bg-deep-purple-100 rounded-lg border-2 border-deep-purple-300 " +
                showEmojis
              }
            >
              <div className="grid gap-1 grid-cols-8 place-items-center text-3xl p-0.5 h-96 overflow-y-auto overflow-x-hidden no-scrollbar">
                {emojiElements}
              </div>
            </div>
            <button
              className="absolute right-2 top-2 bg-deep-purple-300 px-2 py-1 h-10 rounded-md hover:bg-deep-purple-200"
              onClick={send}
            >
              Send
            </button>
