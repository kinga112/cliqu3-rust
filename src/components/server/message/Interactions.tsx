...
// optimized version thanks to gemini
export const Interactions = memo(function Interactions({
  ...
  // OPTIMIZATION: One Effect that only runs when Emojis are actually open
  useEffect(() => {
  ...
  return (
    // Use CSS classes for hover instead of state if possible to reduce JS execution
    <div
      className={`absolute right-5 -bottom-5 w-20 h-10 bg-deep-purple-300 rounded-lg z-10 group-hover:visible ${showEmojis ? "visible" : "invisible"}`}
    >
      <div className="flex h-10 p-1.5 relative gap-2 place-items-center justify-center">
        <button
          className="text-deep-purple-100 border-2 border-deep-purple-100 rounded-lg"
          onClick={
            () => console.log("")
            // setReply({
            //   from: message.from,
        ...
        <button className="text-deep-purple-300" onClick={handleEmojiToggle}>
          <img
            className="select-none"
            src={emoji}
            height={30}
        ...
        {showEmojis && (
          <div
            ref={emojiRef}
            className="z-50 absolute right-0 bottom-10 bg-deep-purple-100 rounded-lg w-fit min-w-max"
          >
            {/*<EmojiGrid
              cid={message.cid}
              from={message.from}
