...
export function DirectMessagesNav() {
  ...
  const setCurrentNavOption = useDirectMessageStore(
    (dm) => dm.setCurrentNavOptions,
  );
  ...
  let requestButtonStyle =
    "p-2 w-full shrink hover:bg-off-black-400 rounded-lg";
  if (currentNavOption == "CHATS") {
    chatButtonStyle =
      "p-2 w-full shrink bg-off-black-300 rounded-lg cursor-default";
  } else {
    requestButtonStyle =
      "p-2 w-full shrink bg-off-black-300 rounded-lg cursor-default";
  }
    ...
    <>
      <div className="flex flex-col gap-2 w-56 shrink-0 bg-off-black-600">
        <div className="flex justify-between place-items-center h-14 border-b z-10 border-off-black-700 shadow-md shadow-off-black-700 shrink-0 p-2">
          <div className="text-2xl font-light">Direct Messages</div>
          <button
            className="bg-off-black-400 hover:bg-off-black-300 p-2 rounded-md"
            onClick={newDM}
          >
        ...
        <div className="flex flex-col p-2 gap-2">
          <div className="flex w-full h-12 gap-2">
            <button
              className={"text-bold " + chatButtonStyle}
              onClick={() => setCurrentNavOption("CHATS")}
            >
              Chats
            </button>
            <button
              className={"text-bold " + requestButtonStyle}
              onClick={() => setCurrentNavOption("REQUESTS")}
            >
              Requests
            </button>
          </div>
