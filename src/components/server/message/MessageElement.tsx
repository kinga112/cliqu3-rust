...
import { XMTPMessage } from "../../../types/messageTypes";
...
function MessageElement(props: {
  ...
  if (props.lastMessage != undefined) {
    if (
      props.message.from == props.lastMessage.from &&
      withinFiveMinutes(
        Number(props.message.timestamp),
        Number(props.lastMessage.timestamp),
      )
    ) {
      shortFormat = true;
    }
  ...
  let picture = currentTextChannel.members[props.message.from].avatar;
  ...
  if (props.message.content_type == "MediaEmbed") {
    imageContent = true;
  }
  ...
  // let reactions = Object.entries(props.message.reactions).map(
  //   ([emoji, reaction]) => {
  //     return reaction.count == 0 ? (
  //       <p />
  //     ) : (
  //       <Reaction
  //         key={emoji + reaction.count}
  //         emoji={emoji}
        ...
        <>
          <div className="flex flex-col gap-2 w-full">
            <span>
              ...
              {found ? (
                <a
                  className="text-blue-400 hover:text-blue-500 inline"
                  href={clickable}
                  target="_blank"
                >
                  {link}
  ...
  function Content() {
            ...
            <div className="fixed inset-0 flex w-screen items-center justify-center">
              <DialogPanel>
                <iframe
                  ...
                  src={"data:application/pdf;base64," + props.base64string}
                />
              </DialogPanel>
            </div>
          </Dialog>
    ...
    if (imageContent == true) {
      return (
        <img className="rounded-md object-scale-down" src={messageContent} />
      );
      ...
      return (
        <div>
          ...
          <button
            className="flex gap-1 py-1 px-2 bg-deep-purple-300 border border-deep-purple-100 rounded-md place-items-center"
  ...
  function MessageFormat() {
    if (shortFormat) {
      const msTimestamp = Number(props.message.timestamp) / 1000000;
      const date = new Date(msTimestamp);
      // const time = new Intl.DateTimeFormat("en-US", {
      //   hour: "numeric",
      //   minute: "2-digit",
      // }).format(Number(props.message.timestamp));
      return (
        <>
          <div className="flex place-items-start gap-2 w-full h-full group">
            <div className="flex text-xxs font-semibold justify-center text-center w-14 shrink-0 p-1 invisible group-hover:visible select-none">
              {date.getHours()}:{date.getMinutes()}
            </div>
            {/*<div
              className={
                "flex flex-col w-full " + (reactions.length == 0 ? "" : "gap-1")
              }
            >
              <div>
                <Content />
              </div>
              <div key={props.message.id} className="flex gap-1">
                {reactions}
              </div>
            </div>*/}
          </div>
      ...
      // // const timeLastMessage = new Intl.DateTimeFormat('en-US', {month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit'}).format(props.message.timestamp)
      const msTimestamp = Number(props.message.timestamp) / 1000000;
      const messageMonth = new Date(msTimestamp).getMonth();
      // console.log("last message: ", props.lastMessage.)
      let lastMessageMonth = -1;
      if (props.lastMessage != undefined) {
        lastMessageMonth = new Date(msTimestamp).getMonth();
      }
      // console.log("MESSAGE MONTH: ", messageMonth, " LAST MESSAGE MONTH: ", lastMessageMonth)
      let time = "";
      if (messageMonth == lastMessageMonth) {
        ...
        // console.log("msTimestamp:", msTimestamp);
        const date = new Date(msTimestamp);
      ...
      return (
        <>
          <div className="flex place-items-start gap-2 w-full h-full">
            <img
              src={picture!}
              className="w-14 h-14 bg-deep-purple-100 rounded-md shrink-0 object-cover select-none pointer-events-none"
            />
            <div
            ...
            >
              <div className="flex flex-row place-items-center gap-2">
                {/*<DisplayName address={props.message.from.slice(0, 5)} />*/}
                <DisplayName
                  profile={currentTextChannel.members[props.message.from]}
                />
                <div className="text-xxs font-semibold select-none">{time}</div>
              </div>
              <div>
                <Content />
              </div>
              {/*<div className="flex gap-1">{reactions}</div>*/}
            </div>
          </div>
        </>
      );
  ...
  return (
    <>
      <div
        ...
        className={
          "flex-col place-items-start relative p-1 hover:bg-off-black-300 gap-2 group rounded mx-5 " +
          (shortFormat ? "" : "mt-4")
        }
      >
        {/*{{props.message.reply ? (
          <div className="flex justify-center gap-3 pl-4">
            ...
            <div className="flex bg-off-black-100 p-2 rounded-sm gap-2 place-items-center">
              {props.message.reply!.from ? (
                <DisplayName
                  address={props.message.reply!.from!.toLowerCase()}
                />
              ) : (
                <p />
              )}
        ...
        <MessageFormat />
        <Interactions key={props.message.id} message={props.message} />
      </div>
    </>
  );
}
