...
export function Reaction(props: {
  ...
  // const addOrRemoveReaction = useServerStore(
  //   (server) => server.addOrRemoveReaction,
  // );
  ...
  async function reactionOnClick() {
    ...
    // addOrRemoveReaction(
    //   props.emoji,
    //   push.api!.account.toLowerCase()!,
    //   props.cid,
    // );

    // await push.api!.chat.send(currentTextChannel.chatId, {
    //   type: "Reaction",
    //   content: props.emoji,
    //   reference: props.cid,
    // });
  ...
  const usersList = props.users.map((user: string, index: number) => {
    ...
    if (props.userProfiles[user.toLowerCase()] == undefined) {
      ...
      if (props.users.length == index + 1) {
        ...
        return (
          <p className="w-max" key={user}>
            {props.userProfiles[user.toLowerCase()].name}
          </p>
        );
      } else {
        // return <p key={user}>{user.substring(0,20)},&nbsp;</p>
    ...
    <>
      <div className="relative">
        <button
          ...
          onMouseEnter={() => setShowUsers("visible")}
          onMouseLeave={() => setShowUsers("invisible")}
        >
          <div>{props.emoji}</div>
          <div>{props.count}</div>
        </button>
        <div
          className={
            "absolute top-7 bg-deep-purple-300 rounded-md max-h-20 border-deep-purple-100 border p-0.5 z-50 " +
            showUsers
          }
        >
          {/* {JSON.stringify(props.reaction.users)} */}
          <div className="flex">{usersList}</div>
        </div>
      </div>
    </>
  );
