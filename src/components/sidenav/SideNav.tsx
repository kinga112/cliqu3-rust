...
import { ServerMetadata } from "../../types/serverTypes";
...
export function SideNav() {
  ...
  async function fetchServers() {
    ...
    if (!result.error) {
      console.log("GET ALL SERVERS DATA: ", result.data);
      if (result.data) {
  ...
  let serverListItems = serverList.map((item: ServerMetadata) => (
    <ServerButton key={item.name + item.creator_address} metadata={item} />
  ));
  ...
  return (
    <>
      <div className="flex relative">
        <div
          id="no-scrollbar"
          className="w-20 overflow-y-scroll shrink-0 pb-20 pt-[132px] overflow-hidden"
        >
