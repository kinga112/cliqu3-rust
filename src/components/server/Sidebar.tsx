...
export function SideBar() {
  ...
  let isCreator = false;
  if (address == currentServer?.metadata.creator_address) {
    isCreator = true;
  }

  // console.log("current server:", currentServer)

  const textChannels = currentServer!.textChannels.map(
    (metadata: TextChannelMetaData) => {
      return <TextChannelButton metadata={metadata} />;
    },
  );
  ...
  function InviteUserModal() {
    ...
    async function sendTicket() {
      ...
      // await push.api!.chat.send(input, {
        ...
        >
          <div className="fixed inset-0 flex w-screen items-center justify-center">
            <DialogPanel className="flex flex-col w-[500px] space-y-1 bg-deep-purple-400 p-12 rounded-md place-items-center">
              <DialogTitle className="flex font-extralight text-5xl w-full py-5 justify-center">
                Invite User
              </DialogTitle>
              <input
                ref={inputRef}
                ...
                className="w-80 border-2 border-deep-purple-100 rounded p-1"
                placeholder="User Wallet Address to Send Ticket"
              />
              <button
                onClick={sendTicket}
                className="w-16 h-8 rounded bg-deep-purple-100 text-deep-purple-300 hover:text-deep-purple-400 font-semibold text-lg"
              >
                Send
              </button>
              {/* <div>
  ...
  function ServerMenu() {
    ...
    function outsideMenuAlerter(
      menuRef: React.MutableRefObject<any>,
      serverNameButtonRef: React.MutableRefObject<any>,
    ) {
      useEffect(() => {
        function handleClickOutside(event: MouseEvent) {
          if (serverNameButtonRef.current) {
    ...
    if (showServerMenu) {
      return (
        <>
          <div
            ref={menuRef}
            className="absolute top-16 left-2 w-52 bg-off-black-400 border border-off-black-300 rounded flex flex-col gap-1 p-2"
          >
            <button
              className="bg-deep-purple-300 hover:bg-deep-purple-400 text-start p-2 rounded"
              onClick={() => {
            ...
            <button className="bg-deep-purple-300 hover:bg-deep-purple-400 text-start p-2 rounded">
              Server Settings
            </button>
          </div>
        </>
  ...
  return (
    <>
      <div
        key={""}
        className="w-56 bg-off-black-600 border-r-1 border-off-black-400 shrink-0"
      >
        <div className="relative">
          <button
        ...
        <div key={""} className="flex flex-col gap-5 overflow-y-auto py-5">
          <div className="flex flex-col gap-1">
            <div className="px-1 flex flex-row place-items-center justify-between group pointer-events-none">
              <div className="flex place-items-center">
                <img
                  className={textChannelArrow}
                  src={carrot}
                  width={20}
                  height={20}
                />
                <button
                  className="p-1 group-hover:text-deep-purple-200 group-hover:underline pointer-events-auto"
              ...
              {isCreator ? (
                <button
                  className="w-6 h-6 rounded-md hover:bg-off-black-200 pointer-events-auto"
                  onClick={() => {
            ...
            {showTextChannels ? (
              ...
              <div>{textChannels}</div>
            ) : (
              <TextChannelButton metadata={currentServer!.textChannels[0]} />
            )}
          ...
          <div className="flex flex-col gap-1">
            ...
            {showVoiceChannels ? <div>{voiceChannels}</div> : <div />}
          </div>
