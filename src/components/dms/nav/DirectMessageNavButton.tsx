...
export function DirectMessageNavButton(props: { chat: any }) {
  ...
  // const appendOldMessages = useServerStore(server => server.appendOldMessages)
    ...
    if (props.chat.chatId != currentDM!) {
      ...
      if (chatInfo?.meta.group) {
        ...
        const recipientProfile = await push.api!.profile.info({
          overrideAccount: chatInfo?.recipient,
        });
  ...
  //   const cachedMesssages = await getMessagesFromCache(props.chat.chatId!);
  ...
  if (typeof props.chat.msg.messageObj.content === "string") {
    ...
    if (
      typeof props.chat.msg.messageObj?.content?.messageObj?.content ===
      "string"
    ) {
  ...
  return (
    <>
      <button
        onClick={changeDM}
        className="bg-off-black-500 px-2 py-1 w-full shrink hover:bg-off-black-400 rounded-lg"
      >
        <div className="flex gap-4 place-items-center">
          <img
            className="w-12 h-12 rounded-md shrink-0 object-cover select-none"
            src={props.chat.profilePicture!}
          />
          <div className="flex flex-col w-32">
            <p className="truncate w-full text-xl text-left">{from}</p>
            <p className="truncate font-light w-full text-white text-opacity-50 text-left">
              {messageContent}
            </p>
...
function Time(props: { timestamp: number }) {
  ...
  return (
    <>
      <p className="text-sm text-left font-light text-white text-opacity-50">
        {timeString}
      </p>
    </>
  );
