...
export function MembersList() {
  ...
  // const adminList = users.admins.map((member: ChatMemberProfile) => {
  //   return <UserInfoSmall key={member.address} member={member} />;
  // });
  // const memberList = currentTextChannel.members.map((member: String) => {
  //   return <UserInfoSmall key={member} member={member} />;
  // });
  //
  //
  // const memberList = currentTextChannel.members.map((member: String) => {
  ...
  // });
  //
  const memberList = Object.entries(currentTextChannel.members).map(
    ([id, profile]) => {
      // return <div key={id}>{profile.address}</div>;
      // console.log("PROFILE:", profile);
      return <UserInfoSmall key={id} profile={profile} />;
    },
  );
  ...
  return (
    <>
      ...
      <div className="flex flex-col z-10 w-48 bg-off-black-500 border-l-2 border-off-black-400 shrink-0 px-2 text-deep-purple-100">
        <button
          className="flex flex-row text-sm pt-2 select-none group pointer-events-none place-items-center select"
          onClick={() => setShowAdmins(!showAdmins)}
        >
          <img
            className={"pointer-events-auto " + adminsArrow}
            src={carrot}
            width={20}
            height={20}
          />
          <div className="group-hover:underline pointer-events-auto">
            admins
          </div>
          {/*<div className="group-hover:underline pointer-events-auto">
            &nbsp;-&nbsp;{adminList.length}
        ...
        {memberList.length != 0 ? (
          <div>
            ...
            >
              <img
                className={"pointer-events-auto " + membersArrow}
                src={carrot}
                width={20}
                height={20}
              />
