...
export function NavItems() {
  ...
  async function fetchChats() {
    ...
    while (true) {
      const fetched = await push.api!.chat.list(currentNavOption, {
        limit: 30,
        page: i,
      });
      if (fetched.length == 0) {
      ...
      fetched.map((chat: IFeeds) => {
        if (
          chat.groupInformation == undefined ||
          chat.groupInformation?.groupDescription == "GROUP DM"
        ) {
  ...
  async function acceptChat(chatId: string) {
    await push.api!.chat.accept(chatId);
  ...
  async function rejectChat(chatId: string) {
    await push.api!.chat.reject(chatId);
    let tempRequests: IFeeds[] = [];
    requests.map((request: IFeeds) => {
      if (chatId != request.chatId) {
        tempRequests.push(request);
  ...
  if (currentNavOption == "CHATS") {
    ...
    const navItems = chats.map((chat: IFeeds) => {
      return <DirectMessageNavButton key={chat.chatId!} chat={chat} />;
    });
    return (
      <>
      ...
      return (
        <div className="flex flex-col gap-0.5">
          ...
          <div className="flex justify-end gap-1">
            <button
              className="p-1 rounded-lg hover:bg-green-900 bg-opacity-75"
              onClick={() => acceptChat(chat.chatId!)}
            >
              Accept
            </button>
            <button
              className="p-1 rounded-lg hover:bg-red-900 bg-opacity-75"
              onClick={() => rejectChat(chat.chatId!)}
            >
              Reject
            </button>
          </div>
