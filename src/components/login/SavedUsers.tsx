...
export function SavedUsers() {
  ...
  const setSavedAddresses = useGlobalStore(
    (globals) => globals.setSavedAddresses,
  );
  ...
  async function getSavedUserData() {
    ...
    if (savedTmpWallet) {
      ...
      localStorage.setItem(
        "tmpWallet",
        JSON.stringify({
          address: pushApi.account,
          key: pushApi.decryptedPgpPvtKey!,
        }),
      );
    }
    ...
    if (accountsJson) {
      ...
      for (const address of accounts) {
        const profile = await pushApi!.profile.info({
  ...
  async function loginSavedUser(address: string) {
    ...
    if (key) {
      ...
      const result = await tryCatch(
        invoke("login", {
          user: pushApi.account,
          pgpKey: pushApi.decryptedPgpPvtKey!,
        }),
      );
      if (!result.error) {
        console.log("Successfully stored user pgp");
      } else {
        console.log("Error:", result.error);
      }
      // Cookies.set('session_token', 'test_token_12345', { expires: 365 })
      Cookies.set("session_token", "test_token_12345");
  ...
  const savedAccountsList = savedAccounts.map(
    (account: UserProfile, index: number) => {
      return (
        <button
          key={savedAddresses[index]}
          onClick={() => loginSavedUser(savedAddresses[index])}
        >
          <UserInfoLarge
  ...
  return (
    <>
      ...
      >
        <button onClick={() => setShowSavedUsers(false)}>
          <img
            className="absolute right-5 top-1/2 z-10 w-14 h-14 rotate-180"
            src={arrow}
          />
        </button>
