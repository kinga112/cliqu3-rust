...
export function CreateServerModal() {
  const showCreateServerModal = useGlobalStore(
    (globals) => globals.showCreateServerModal,
  );
  ...
  async function createServer() {
    ...
    if (name == "") {
      ...
      const result = await tryCatch(
        invoke("create_server", { name, pic, creatorAddress }),
      );
      if (!result.error) {
        console.log("Created Server: ", result.data);
        // if (typeof result.data === "string") {
        //   // await push.createTextChannel(result.data, 'general', 'text channel', [])
        //   await xmtp.createTextChannel(result.data, [], "general");
        // }
      } else {
        console.log("Error creatring server: ", result.error);
      }
    }
  }
        ...
        <div className="flex flex-col p-10 max-w-lg space-y-1 bg-deep-purple-300 text-deep-purple-100 select-none">
          ...
          <div className="flex gap-2">
            <div className="bg-deep-purple-100 rounded-xl w-32 h-32">
              {pic != "" ? (
                <img className="w-32 h-32 rounded-xl object-cover" src={pic} />
              ) : (
                <div />
              )}
            </div>
            <div className="flex flex-col gap-1">
              <input
                className="bg-deep-purple-400 p-2 rounded-md "
              ...
              <input
                className="bg-deep-purple-400 p-2 rounded-md"
                placeholder="icon image address link"
          ...
          <div className="flex gap-2 pt-2">
            <button
              className="bg-deep-purple-400 text-deep-purple-100 p-2 w-32 rounded-md duration-100 shadow-off-black-300 shadow-md hover:shadow-none hover:border-deep-purple-100"
              onClick={() => {
                createServer();
              }}
            >
            ...
            <button
              className="bg-slate-900 p-2 w-20 rounded-md hover:text-red-400"
              onClick={() => {
