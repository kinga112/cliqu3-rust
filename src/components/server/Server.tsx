    ...
    (async () => {
      try {
        unlisten = await listen<any>("iroh_event", async (event) => {
          ...
          console.log(
            "current screen inside iroh event listener: ",
            currentServer,
          );
          if (event.payload.data == "missing data") {
            console.log(
              "MISSING DATA NEED TO RELOAD SERVER!:",
              currentServer?.metadata.id,
            );
            const result = await tryCatch(
              invoke("get_server", { id: currentServer?.metadata.id }),
            );
            if (!result.error) {
            ...
            const voiceChannels: { [id: string]: VoiceChannel } =
              event.payload.data;
            ...
            const server: ServerType = {
              ...
              addChannelModalVisibility:
                currentServer!.addChannelModalVisibility,
            };
            // const server: ServerType = {
